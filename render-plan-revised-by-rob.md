...
- Is human-auditable: a person familiar with both Lean and the LF encoding
  ...
  encoding of what was in the NDJSON, and the holes represent proof
  obligations that a valid kernel checker is going to need to verify in order
  to only accept valid proofs."

Probably we can make the output of this pass compile as valid Twelf easily, by
just having the render pass break the not-enforced-by-Twelf rule of "only emit
Twelf declarations of `declared` facts, everything else must be a definition."
However, that's not the priority or a requirement.
...
There may be some NDJSON files we _cannot_ fairly represent in JSON: for
instance, if an NDJSON uses de Bruijn indices in an invalid way, we are not
obligated to present a render-pass output for this NDJSON. The render pass
code itself should be clean, well-architected, and auditable by a human, so
humans can review it and convince themselves that this is a fair Twelf
representation of the NDJSON files, that any valid NDJSON file is
representable in Twelf in this way, and that any NDJSON file we do not render
in Twelf is non-renderable because Twelf can't capture the way in which the
NDJSON is broken.
...
- `render.ts` is **total** (or nearly so) over the JSON IR. It never throws.
...
better long-term; the second (parallel) is safer to land incrementally. The
refactor has the advantage of creating a "trusted code base" — we're relying
on render.ts to truthfully give us a Twelf file, but the pass that tries to
fill in proof obligations can be a complete oyster carnival: any proof
obligation it tries to resolve is going to be

## Hole conventions

Output is roughly Twelf, but holes are markers rather than necessarily valid
Twelf terms. Two flavors:
...
exactly what proof obligation is unmet. We _could_ declare without proof a
`?HOLE_defeq?` const before this expression and Twelf would accept the file...
but this is a nice-to-have and not a priority.
...
HOLE/Foo_mk/positivity : ctor-positive "Foo" lnil <T_lf>.
```
...
(As above: this could be done better with constraint domains in Twelf.)

Current `Expr` already has `proj`, `natLit`, `strLit` cases (added during the
parse split). They just throw in `lfExpr`. No type changes needed. This could
be improved by using Twelf's constraint domains to actually capture numeric
literals and strings — the `first-version.elf` development did just this.
...
- **Good 🤷 cannot-represent**: We don't have a way to represent this in Twelf
  currently.
...
- **Bad 🤷 cannot-represent**: We don't have a way to represent this in Twelf
  currently. We might not ever (for instance, if it's an NDJSON that uses de
  bruijn indices in an invalid way).
...
Some bad tests are GOING TO NECESSARILY REMAIN with-holes permanently, because
they contain proof obligations that _can't possibly be discharged_. That's ok!
We don't know if there are any bad tests that must remain in the 🤷 category
forever
